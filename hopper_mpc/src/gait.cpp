...
Gait::Gait(Model model, double dt, Eigen::Matrix<double, 13, 1> X_f) {
  ...
  ki << 0.2, 0.2, 0;
  kd << 0.01, 0.02, 0;
  pid_dpPtr_.reset(new PID3(dt_, kp, ki, kd));

  z_ref = 0;
  pe_ref << 0, 0, model_.h0;
  pf_ref.setZero();
}
...
Eigen::Matrix<double, 5, 1> Gait::uRaibert(std::string state, std::string state_prev, Eigen::Matrix<double, 13, 1> X_in,
  ...
  Eigen::Vector3d dp_ref = -Q_z.matrix() * pid_dpPtr_->PIDControl(R_z * p, R_z * p_ref);  // adjust for yaw, rotate back world->body
  ...
  if (state == "Rise") {                                       // in first timestep after liftoff,
    ...
    pe_ref(2) = model_.h0;  // pull leg up to prevent stubbing
  } else if (state == "Fall") {
    pe_ref(2) = model_.h0 * 5.5 / 3;  // brace for impact
  } else if (state == "Cmpr") {
    pe_ref(2) = model_.h0 * 4.5 / 3;  // TODO: Try reducing gains for compression instead of changing position setpoint
  } else if (state == "Push") {
    pe_ref(2) = model_.h0 * 5.5 / 3;  // pushoff
  }

  Q_ref.setFromTwoVectors(pf_ref, p);  // Q_ref = utils.Q_inv(utils.vec_to_quat(self.x_des - p))
  u.block<2, 1>(0, 0) = legPtr_->KinInvPosCtrl(pe_ref);
  u.block<3, 1>(2, 0) = rwaPtr_->AttitudeCtrl(Q_ref, Q_base, z_ref);
  return u;
}
