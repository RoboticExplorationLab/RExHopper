...
void NormaliseRotmat(Rotmat& R) {
  ...
  if (R.determinant() < 0.0) R.setIdentity();
}
...
Vec3 RandVec(double maxNorm) {
  ...
  if (vecNorm > 0.0) vec *= desNorm / vecNorm;
  return vec;
}
...
bool ValidateFused(FusedAngles& f, double tol, bool unique) {
  ...
  if (unique) {
    ...
    if (sineSum >= 1.0 - tol) f.hemi = true;
    L1Norm = fabs(f.fusedPitch) + fabs(f.fusedRoll);
    if (L1Norm <= tol && !f.hemi) f.fusedYaw = 0.0;
  }

  // Return whether the fused angles were valid within the given tolerance
  return (fabs(f.fusedYaw - forig.fusedYaw) <= tol && fabs(f.fusedPitch - forig.fusedPitch) <= tol &&
          fabs(f.fusedRoll - forig.fusedRoll) <= tol && f.hemi == forig.hemi);
}
...
bool ValidateTilt(TiltAngles& t, double tol, bool unique) {
  ...
  if (unique) {
    ...
    if (near0 || near180) t.tiltAxisAngle = 0.0;
    if (near180) t.fusedYaw = 0.0;
  }

  // Return whether the tilt angles were valid within the given tolerance
  return (fabs(t.fusedYaw - torig.fusedYaw) <= tol && fabs(t.tiltAxisAngle - torig.tiltAxisAngle) <= tol &&
          fabs(t.tiltAngle - torig.tiltAngle) <= tol);
}
...
bool QuatEqual(const Quat& qa, const Quat& qb, double tol) {
  ...
  bool isSame =
      (fabs(qa.w() - qb.w()) <= tol && fabs(qa.x() - qb.x()) <= tol && fabs(qa.y() - qb.y()) <= tol && fabs(qa.z() - qb.z()) <= tol);
  bool isOpp =
      (fabs(qa.w() + qb.w()) <= tol && fabs(qa.x() + qb.x()) <= tol && fabs(qa.y() + qb.y()) <= tol && fabs(qa.z() + qb.z()) <= tol);
  return (isSame || isOpp);
}
...
bool EulerEqual(const EulerAngles& ea, const EulerAngles& eb, double tol) {
  ...
  return (fabs(eau.yaw - ebu.yaw) <= tol && fabs(sin(eau.pitch) - sin(ebu.pitch)) <= tol &&
          fabs(eau.roll - ebu.roll) <= tol);  // The pitch suffers from the numerical insensitivity of asin, so the sine thereof is checked
}
...
bool FusedEqual(const FusedAngles& fa, const FusedAngles& fb, double tol) {
  ...
  return (fabs(fau.fusedYaw - fbu.fusedYaw) <= tol && fabs(sin(fau.fusedPitch) - sin(fbu.fusedPitch)) <= tol &&
          fabs(sin(fau.fusedRoll) - sin(fbu.fusedRoll)) <= tol &&
          fau.hemi ==
              fbu.hemi);  // The fused pitch and roll suffer from the numerical insensitivity of asin, so the sine's thereof are checked
}

// Check equality: Fused angles (exact)
bool FusedEqualExact(const FusedAngles& fa, const FusedAngles& fb, double tol) {
  // Return whether to the specified tolerance the fused angles are the same
  return (fabs(fa.fusedYaw - fb.fusedYaw) <= tol && fabs(fa.fusedPitch - fb.fusedPitch) <= tol &&
          fabs(fa.fusedRoll - fb.fusedRoll) <= tol && fa.hemi == fb.hemi);
}
...
bool TiltEqual(const TiltAngles& ta, const TiltAngles& tb, double tol) {
  ...
  return (fabs(tau.fusedYaw - tbu.fusedYaw) <= tol && fabs(stiltasq * cos(tau.tiltAxisAngle) - stiltbsq * cos(tbu.tiltAxisAngle)) <= tol &&
          fabs(stiltasq * sin(tau.tiltAxisAngle) - stiltbsq * sin(tbu.tiltAxisAngle)) <= tol &&
          fabs(cos(tau.tiltAngle) - cos(tbu.tiltAngle)) <=
              tol);  // The tilt angle suffers from the numerical insensitivity of acos, so the cosine thereof is checked / The tilt axis
                     // angle has a singularity when the tilt angle is zero, so two geometrically relevant terms are checked instead of the
                     // tilt axis angle directly
}
...
bool TiltEqualExact(const TiltAngles& ta, const TiltAngles& tb, double tol) {
  ...
  return (fabs(ta.fusedYaw - tb.fusedYaw) <= tol && fabs(ta.tiltAxisAngle - tb.tiltAxisAngle) <= tol &&
          fabs(ta.tiltAngle - tb.tiltAngle) <= tol);
}
...
void RotmatWithEYaw(const Rotmat& R, double eulerYaw, Rotmat& Rout) {
  ...
  Rotmat REYawAdj;
  REYawAdj << cdEYaw, -sdEYaw, 0.0, sdEYaw, cdEYaw, 0.0, 0.0, 0.0, 1.0;
...
void RotmatWithFYaw(const Rotmat& R, double fusedYaw, Rotmat& Rout) {
  ...
  // Construct the yaw adjustment rotation
  Rotmat RFYawAdj;
  RFYawAdj << cdFYaw, -sdFYaw, 0.0, sdFYaw, cdFYaw, 0.0, 0.0, 0.0, 1.0;
...
Vec3 QuatRotVecPureZ(const Quat& q, double vz) {
  // Calculate and return the required vector
  return Vec3(vz * 2.0 * (q.x() * q.z() + q.y() * q.w()), vz * 2.0 * (q.y() * q.z() - q.x() * q.w()),
              vz * (1.0 - 2.0 * (q.x() * q.x() + q.y() * q.y())));
}
...
void RotmatRotGlobalX(const Rotmat& R, double angle, Rotmat& Rout) {
  ...
  Rout << R.coeff(0, 0), R.coeff(0, 1), R.coeff(0, 2), R.coeff(1, 0) * cang - R.coeff(2, 0) * sang,
      R.coeff(1, 1) * cang - R.coeff(2, 1) * sang, R.coeff(1, 2) * cang - R.coeff(2, 2) * sang, R.coeff(2, 0) * cang + R.coeff(1, 0) * sang,
      R.coeff(2, 1) * cang + R.coeff(1, 1) * sang, R.coeff(2, 2) * cang + R.coeff(1, 2) * sang;
}
...
void RotmatRotGlobalY(const Rotmat& R, double angle, Rotmat& Rout) {
  ...
  // Calculate the required rotated rotation
  Rout << R.coeff(0, 0) * cang + R.coeff(2, 0) * sang, R.coeff(0, 1) * cang + R.coeff(2, 1) * sang,
      R.coeff(0, 2) * cang + R.coeff(2, 2) * sang, R.coeff(1, 0), R.coeff(1, 1), R.coeff(1, 2), R.coeff(2, 0) * cang - R.coeff(0, 0) * sang,
      R.coeff(2, 1) * cang - R.coeff(0, 1) * sang, R.coeff(2, 2) * cang - R.coeff(0, 2) * sang;
}
...
void RotmatRotGlobalZ(const Rotmat& R, double angle, Rotmat& Rout) {
  ...
  // Calculate the required rotated rotation
  Rout << R.coeff(0, 0) * cang - R.coeff(1, 0) * sang, R.coeff(0, 1) * cang - R.coeff(1, 1) * sang,
      R.coeff(0, 2) * cang - R.coeff(1, 2) * sang, R.coeff(1, 0) * cang + R.coeff(0, 0) * sang, R.coeff(1, 1) * cang + R.coeff(0, 1) * sang,
      R.coeff(1, 2) * cang + R.coeff(0, 2) * sang, R.coeff(2, 0), R.coeff(2, 1), R.coeff(2, 2);
}
...
void RotmatFromAxis(UnitAxis axis, double angle, Rotmat& R) {
  ...
  if (axis == X_AXIS) {
    R << 1.0, 0.0, 0.0, 0.0, cang, -sang, 0.0, sang, cang;
  } else if (axis == Y_AXIS) {
    R << cang, 0.0, sang, 0.0, 1.0, 0.0, -sang, 0.0, cang;
  } else  // Z_AXIS
  {
    R << cang, -sang, 0.0, sang, cang, 0.0, 0.0, 0.0, 1.0;
  }
}
...
void RotmatFromAxis(const Vec3& axis, double angle, Rotmat& R) {
  ...
  // Calculate the required rotation matrix
  R << xxC + cang, xyC - zs, zxC + ys, xyC + zs, yyC + cang, yzC - xs, zxC - ys, yzC + xs, zzC + cang;
}
...
Rotmat RotmatFromEuler(double yaw, double pitch, double roll) {
  ...
  Rotmat R;
  R << cpsi * cth, cpsi * sth * sphi - spsi * cphi, cpsi * sth * cphi + spsi * sphi, spsi * cth, spsi * sth * sphi + cpsi * cphi,
      spsi * sth * cphi - cpsi * sphi, -sth, cth * sphi, cth * cphi;
  return R;
}
...
Quat QuatFromEuler(double yaw, double pitch, double roll) {
  ...
  return Quat(hcphi * hcth * hcpsi + hsphi * hsth * hspsi, hsphi * hcth * hcpsi - hcphi * hsth * hspsi,
              hcphi * hsth * hcpsi + hsphi * hcth * hspsi, hcphi * hcth * hspsi - hsphi * hsth * hcpsi);  // Order: (w,x,y,z)
}
...
Rotmat RotmatFromFused(double fusedPitch, double fusedRoll)  // Assume: fusedYaw = 0, hemi = true
{
  ...
  // Calculate and return the required rotation matrix
  Rotmat R;
  R << A, C, sth, C, B, -sphi, -sth, sphi, calpha;
  return R;
}
...
Rotmat RotmatFromFused(double fusedYaw, double fusedPitch, double fusedRoll, bool hemi) {
  ...
  // Calculate and return the required rotation matrix
  Rotmat R;
  R << A + D * calpha, B - C * calpha, salpha * spsigam, C - B * calpha, D + A * calpha, -salpha * cpsigam, -sth, sphi, calpha;
  return R;
}
...
EulerAngles EulerFromFused(double fusedYaw, double fusedPitch, double fusedRoll, bool hemi) {
  ...
  double crit = sth * sth + sphi * sphi;
  double calpha = 0.0;
  if (crit < 1.0) calpha = (hemi ? sqrt(1.0 - crit) : -sqrt(1.0 - crit));
...
Rotmat RotmatFromTilt(double fusedYaw, double tiltAxisAngle, double tiltAngle) {
  ...
  // Calculate and return the required rotation matrix
  Rotmat R;
  R << A + D * calpha, B - C * calpha, salpha * spsigam, C - B * calpha, D + A * calpha, -salpha * cpsigam, -sgam * salpha, cgam * salpha,
      calpha;
  return R;
}
...
Quat QuatSlerp(const Quat& q0, const Quat& q1, double u) {
  ...
  if (dprod >=
      1.0 -
          5e-9)  // A dot product within this tolerance of unity produces a negligible amount of error if using linear interpolation instead
  {
...
Quat QuatSlerp(const Quat& q, double u) {
  ...
  if (qu.w() >=
      1.0 -
          5e-9)  // A w component within this tolerance of unity produces a negligible amount of error if using linear interpolation instead
  {
    // Perform the required interpolation
    qu *= u;
    qu.w() += (1.0 - u);
...
Quat QuatHFromFYawGTiltH(const Quat& qGH, double fusedYawG, const Quat& qH) {
  ...
  if (F < 64.0 * DBL_EPSILON)
    ...
    double chphi = G / F;
    double shphi = H / F;
    return Quat(chphi * qH.w() - qH.z() * shphi, chphi * qH.x() - qH.y() * shphi, chphi * qH.y() + qH.x() * shphi,
                chphi * qH.z() + qH.w() * shphi);  // Order: (w,x,y,z)
  }
}
}  // namespace rot_conv
   // EOF